...
class UsersTestCase(IgorApiTestCase):
    ...
    def test_list_users(self):
        ...
        response = self.client.get(url_for('users'), headers=self.headers)
    ...
    def test_user_info(self):
        ...
        response = self.client.get(url_for('user', username=self.test_user),
                                   headers=self.headers)
        self.assert_200(response)
    ...
    def test_nonexistent_user_info(self):
        response = self.client.get(url_for('user', username='something'),
                                   headers=self.headers)
        self.assert_404(response)

    def test_remove_root_user(self):
        self.create_test_user()
        response = self.client.delete(url_for('user',
                                              username=ROOT_USER),
                                      headers=[('Authorization', 'Basic '
                                        + base64.b64encode(self.test_user +
                                                           ':' +
    ...
    def test_update_user(self):
        ...
        response = self.client.put(url_for('user', username=self.test_user),
                                   data=data,
                                   headers=self.headers)
        self.assert_200(response)
